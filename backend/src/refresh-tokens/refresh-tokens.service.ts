...
export class RefreshTokensService {
  ...
  async reissueToken(
    token: string,
  ): Promise<{ refreshToken: string; user: AuthorizedUser }> {
    ...
    try {
      return await this.dataSource.transaction(async (db) => {
        ...
        if (existing?.revoked) {
          ...
          this.logger.warn(
            `SECURITY: Refresh token reuse detected userId: ${existing.user.id}`,
          );
        ...
        const newEncodedToken = this.encodeToken(
          this.generateToken(),
          persistent,
        );
      ...
      if (revokeAllForUserId) {
        try {
          ...
          this.logger.error(
            `Error revoking token for userId: ${revokeAllForUserId}`,
            error,
          );
        }
      }
